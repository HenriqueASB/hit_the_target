...
void OpenGLWidget::initializeGL(){
    ...
    glClearColor(0,0,0,1);   // R G B Alpha(Opacity)

    qDebug("open gl %s",glGetString(GL_VERSION));
...
void OpenGLWidget::explosion(bool hit){
    makeCurrent();
    if(hit){
...
void OpenGLWidget::paintGL(){
    ...
    //projectile
    if(shooting2){
        glUniform4f(locTranslation,projectilePos2[0],projectilePos2[1],0,0);
        glUniform1f(locScaling,0.08f);
        glDrawElements(GL_TRIANGLES,indices.size(),GL_UNSIGNED_INT,0);
    }
...
void OpenGLWidget::createShaders(){
    ...
    if(!vs.open((QFile::ReadOnly | QFile::Text)))return;//mensagem de erro
    if(!fs.open((QFile::ReadOnly | QFile::Text)))return;//mensagem de erro
   ...
   if(isLinked==GL_FALSE){
       ...
       std::vector<GLchar> inforLog(maxLength);
       glGetProgramInfoLog(shaderProgram,maxLength,&maxLength,&inforLog[0]);
       qDebug("%s",&inforLog[0]);
       glDeleteShader(fragmentShader);
       glDeleteShader(vertexShader);
...
void OpenGLWidget::createVBOs(){
    ...
    colors[0] = QVector4D(0.46,0.53,0.6,1);
    colors[1] = QVector4D(0.46,0.53,0.6,1);
    colors[2] = QVector4D(0.46,0.53,0.6,1);
    colors[3] = QVector4D(0.46,0.53,0.6,1);
...
void OpenGLWidget::keyPressEvent(QKeyEvent *event){
    switch (event->key()) {
        ...
        break;
        case Qt::Key_Space:
           if(!shooting && !rapidShooting){
               shooting = true;
               projectilePos[0] = -0.7f;
               projectilePos[1] = playerPosY;
           }
           if((rand()%10)>8 && !shooting2){
               shooting2 = true;
               projectilePos2[0] = 0.7f;
               projectilePos2[1] = targetPosY;
           }
        break;
        case Qt::Key_C:
           if(!shooting && !rapidShooting){
...
void OpenGLWidget::animate(){
    ...
    if(shooting){
        ...
        projectilePos[0] += 4.0f*elTime;
        if(projectilePos[0] > 0.8f){
            if(std::fabs(projectilePos[1] - targetPosY)<0.05f){
                numHits++;
                qDebug("Hit!!!!");
                emit updateHitsLabel(QString("#Hits: %1").arg(numHits));
                explosion(true);
    ...
    //projectile2
    if(shooting2){
        projectilePos2[0] += -3.0f*elTime;
        if(projectilePos2[0] < -0.8f){
            if(std::fabs(projectilePos2[1] - playerPosY)<0.125f){
                numHits--;
                emit updateHitsLabel(QString("#Hits: %1").arg(numHits));
                explosion(true);
                shooting2 = false;
